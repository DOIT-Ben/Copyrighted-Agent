(() => {
  ...
  const renderLoadingButton = (button, label) => {
    clearChildren(button);
    const spinner = document.createElement("span");
    spinner.className = "button-spinner";
    spinner.setAttribute("aria-hidden", "true");
    const text = document.createElement("span");
    text.textContent = String(label || "正在处理，请稍候");
  ...
  const pollAsyncJob = async (statusUrl, redirectUrl, inlineStep, pendingDetail) => {
    const safeStatusUrl = normalizeLocalUrl(statusUrl, "");
    const safeRedirectUrl = normalizeLocalUrl(redirectUrl, "/submissions");
    if (!safeStatusUrl) {
      throw new Error("任务状态地址异常，请刷新页面后重试。");
    }

    for (;;) {
      const jobResponse = await window.fetch(safeStatusUrl, { headers: { Accept: "application/json" } });
      if (!jobResponse.ok) {
      ...
      if (jobPayload.status === "failed" || jobPayload.status === "interrupted") {
        ...
        throw Object.assign(new Error(hint), { jobPayload, redirectUrl: safeRedirectUrl });
      }
      await wait(900);
    }
  };
    ...
    form.addEventListener("submit", async (event) => {
      ...
      for (const button of submitButtons) {
        ...
        if (button.tagName === "BUTTON") {
          ...
          renderLoadingButton(button, button.dataset.pendingLabel || pendingText);
        }
      }

      const asyncUrl = normalizeLocalUrl(form.dataset.asyncUploadUrl, "");
      ...
      try {
        ...
        const statusUrl = normalizeLocalUrl(
          submitPayload.status_url || (submitPayload.job_id ? "/api/jobs/" + submitPayload.job_id : ""),
          "",
        );
        const redirectUrl = normalizeLocalUrl(
          submitPayload.redirect_url || (submitPayload.submission_id ? "/submissions/" + submitPayload.submission_id : "/submissions"),
          "/submissions",
        );
        stopStageTimer();
        ...
        if (jobPayload && jobPayload.can_retry && jobPayload.retry_url) {
          actions.unshift({
            ...
            onClick: async () => {
              ...
              try {
                const retryUrl = normalizeLocalUrl(jobPayload.retry_url, "");
                if (!retryUrl) {
                  throw new Error("重试地址异常，请刷新页面后重试。");
                }
                const retryResponse = await window.fetch(retryUrl, { method: "POST", headers: csrfHeaders });
                if (!retryResponse.ok) {
                  throw new Error(await readErrorMessage(retryResponse));
                }
                const retryPayload = await retryResponse.json();
                await pollAsyncJob(
                  retryPayload.status_url || "",
                  retryPayload.redirect_url || redirectUrl || "/submissions",
                  inlineStep,
                  pendingDetail,
                );
              } catch (retryError) {
